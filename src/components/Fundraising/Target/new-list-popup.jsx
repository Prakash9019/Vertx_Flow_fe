...
export default function NewListPopup({ isOpen, onClose, onSave }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      <div
        className="relative flex flex-col items-center justify-center bg-cover bg-center bg-no-repeat
                   w-full max-w-sm sm:max-w-md md:max-w-lg lg:max-w-2xl xl:max-w-[43.75rem]
                   max-h-[calc(100vh-2rem)] sm:max-h-[calc(100vh-3rem)] overflow-y-auto rounded-lg py-8 sm:py-10 md:py-12"
        style={{
          backgroundImage: `url(${Rectangle82Placeholder})`, // Using placeholder
        }}
      >
        {/* Header text */}
        <div className="text-center px-4"> {/* Added px for smaller screens */}
          <h2 className="text-white text-xl sm:text-2xl font-semibold mb-2 font-['Inter']">Let's create a new target list.</h2>
          <p className="text-[#B8B8B8] text-center font-['Inter'] text-xs font-normal max-w-[25rem] mx-auto leading-[1.4]">
            A target list is a curated set of investors for your fundraise.
            <br />
            You can edit and share it anytime, unless it was created by Vertx.
        ...
        {/* Main form container */}
        {/* Responsive width and height adjustments */}
        <div className="w-full max-w-xs sm:max-w-sm md:max-w-md lg:max-w-lg xl:max-w-[40rem]
                       min-h-[18.75rem] rounded-[0.3125rem] bg-black/76 relative mt-6 p-6 sm:p-8 flex flex-col justify-between">
          {/* Name the target list section */}
          ...
          {/* Choose the cover section */}
          <div className="mb-auto"> {/* Used mb-auto to push cover options down slightly, ensuring button space */}
            <h3 className="text-white font-['Inter'] text-base font-medium mb-4">Choose the cover</h3>

            <div className="flex flex-wrap gap-3 justify-center sm:justify-start"> {/* Added flex-wrap for options */}
              {coverOptions.map((option) => (
                <div
                  key={option.id}
                  onClick={() => setSelectedCover(option.id)}
                  className="cursor-pointer w-10 h-10 rounded-[0.125rem] flex items-center justify-center transition-transform duration-200 flex-shrink-0"
                  style={{
                    background: option.color,
                    border:
                      option.id === "default" ? option.border : selectedCover === option.id ? "2px solid #FFF" : "none", // Explicitly "none"
                    transform: selectedCover === option.id ? "scale(1.2)" : "scale(1)",
                  }}
                >
                  {option.id === "default" && (
          ...
          {/* Buttons - positioned at the bottom right using flexbox */}
          {/* Removed absolute positioning for better responsiveness */}
          <div className="flex justify-end items-center gap-2 mt-8"> {/* Added top margin for separation */}
            <button
              onClick={handleCancel}
              className="w-20 h-8 bg-white/20 rounded-[0.125rem] border-none cursor-pointer text-white transition-colors hover:bg-gray-500"
            >
