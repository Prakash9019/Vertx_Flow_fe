// Vertx_Flow_fe/src/screens/LocationSelect.jsx
import React, { useEffect } from "react";
import { useNavigate } from "react-router-dom";
import Header from "../components/Header";
import { useStartupProfile } from "../context/StartupProfileContext";
// import RightIcon from "../assets/RightTick.svg"; // Only needed for step display if different

const LocationSelect = () => { // Renamed to PascalCase
  const navigate = useNavigate();
  const { startupData, loadingData, error, setError } = useStartupProfile();

  const selectedLocation = startupData.location || 'Not specified'; // Get from context

  const handleContinue = () => {
    navigate('/profile/raise'); // Navigate to RaiseFunds (main input for raise)
  };

  const handleBack = () => {
    navigate('/profile/location'); // Navigate back to LocationSetup (main input for location)
  };
          ...
          <div className="relative z-10">
            ...
            <div className="max-w-2xl mx-auto mt-10">
                <h2 className="text-3xl font-semibold mb-2 text-center">
                    Confirm Startup Location
                </h2>
                <p className="text-center text-gray-400 mb-8">
                    You have selected the following location.
                </p>
                <div className="bg-[#0E0E11] p-6 sm:p-8 rounded-xl shadow-2xl border border-gray-700">
                    <div className="flex justify-around items-center mb-8 text-xs">
                        {['Stage', 'Location', 'Raise', 'Revenue', 'Industry', 'Pitch'].map((stepLabel, index) => (
                            <div key={stepLabel} className={`flex flex-col items-center ${index <= 1 ? 'text-purple-400' : 'text-gray-500'}`}>
                            <div className={`w-6 h-6 rounded-full flex items-center justify-center border-2 ${index <= 1 ? 'bg-purple-500 border-purple-500' : 'border-gray-500'}`}>
                                {index + 1}
                            </div>
                            <span>{stepLabel}</span>
                            </div>
