...
const RevenueStaus = () => {
  ...
  //   navigate('/revenue-select', { state: { selectedStage, selectedRevenue } });
  // };

  const handleContinue = async () => {
    if (!selectedAmount) {
      setError(
        "Please select the amount you're looking to raise before continuing."
      );
      return;
    }
    try {
      await axios.post("http://localhost:5009/api/startup", {
    ...
    <div className="w-[1580px] h-[832px] bg-gradient-to-b from-[#0F0C29] via-[#08080d] to-[#0b0b3f] text-white flex flex-col items-center justify-center p-6">
      ...
      <div className="relative w-[720px] h-[362px] mt-[15px] mr-[180px] gradient-border rounded-[10px] bg-black text-white p-8">
        ...
        <Dropdown
          ...
          onSelect={(value) => setSelectedRevenue(value)}
        />
        {/* <select
        ...
        <div className="absolute flex justify-between items-center w-[630px] mt-[86.5px]">
          {" "}
          {/* Use w-full to take full width */}
          {/* Back Button */}
          <button
            className="relative w-[33px] h-[17px] font-inter font-normal text-[14px] leading-[100%] text-white"
            onClick={() => navigate("/raise-select")}
          >
            Back
          </button>
          {/* Continue Button */}
          <button
            className="w-[100px] h-[36px] font-inter text-[14px] font-medium bg-white text-black rounded-[4px] ml-auto"
